...
!=============================================================================== SK from KK 20200408
!-----------------------------------------------------------------------
! CRPS Spread Skill (CRSP)  ! SK 20240805
! Supplementary Information of Neural GCM (Kochkov et al. 2024; Nature)
! https://static-content.springer.com/esm/art%3A10.1038%2Fs41586-024-07744-y/MediaObjects/41586_2024_7744_MOESM1_ESM.pdf
!-----------------------------------------------------------------------
SUBROUTINE com_crsp(ndim, var1, crsp)
  IMPLICIT NONE
  INTEGER, INTENT(IN)      :: ndim
  REAL(r_size),INTENT(IN)  :: var1(ndim)
  REAL(r_size),INTENT(OUT) :: crsp
  INTEGER :: i,j

  crsp = 0.d0
  DO j=1,ndim
    DO i=1,ndim
      crsp=crsp + ( (var1(i) - var1(j))**2.0d0 )
    END DO
  END DO 
  crsp=dsqrt(crsp)/dble(2*ndim*(ndim-1))

  RETURN
END SUBROUTINE com_crsp
!-----------------------------------------------------------------------
