...
CONTAINS
SUBROUTINE set_common_mpi_XXXXXX
  ...
  WRITE(6,'(A)') 'Hello from set_common_mpi_XXXXXX'
  ...
  RETURN
END SUBROUTINE set_common_mpi_XXXXXX
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
SUBROUTINE read_ens_mpi(base,member,v3d,v2d)
  ...
  REAL(r_sngl) :: v2dg(nlon,nlat,nv2d)
  INTEGER :: l,n,ll,im
  CHARACTER(256) :: filename='gues/MMMMMM/yyyymmddhh.grd'

  ll = CEILING(REAL(member)/REAL(nprocs))
  DO l=1,ll
    im = myrank+1 + (l-1)*nprocs
    IF(im <= member) THEN
      WRITE(filename,'(2A,I6.6,A,I10.10,A)') trim(base),"/",im,"/",ymdh,".grd"
      WRITE(6,'(A,I3.3,2A)') 'MYRANK ',myrank,' is reading a file ',TRIM(filename)
      CALL read_grd4(TRIM(filename),v3dg,v2dg)
    END IF
...
!-----------------------------------------------------------------------
SUBROUTINE write_ens_mpi(base,member,v3d,v2d)
  CHARACTER(*),INTENT(IN) :: base
  INTEGER,INTENT(IN) :: member
  ...
  REAL(r_sngl) :: v2dg(nlon,nlat,nv2d)
  INTEGER :: l,n,ll,im
  CHARACTER(256) :: filename='anal/MMMMMM/yyyymmddhh.grd'
  ...
  DO l=1,ll
    ...
    IF(im <= member) THEN
      WRITE(filename,'(2A,I6.6,A,I10.10,A)') trim(base),"/",im,"/",ymdh,".grd"
      WRITE(6,'(A,I3.3,2A)') 'MYRANK ',myrank,' is writing a file ',TRIM(filename)
      CALL write_grd4(TRIM(filename),v3dg,v2dg)
    END IF
  END DO
...
!-----------------------------------------------------------------------
SUBROUTINE write_ensmspr_mpi(base,member,v3d,v2d)
  CHARACTER(*),INTENT(IN) :: base
  INTEGER,     INTENT(IN) :: member
  REAL(r_size),INTENT(IN) :: v3d(nij1,nlev,member,nv3d)
  REAL(r_size),INTENT(IN) :: v2d(nij1,member,nv2d)
  ...
  REAL(r_sngl) :: v2dg(nlon,nlat,nv2d)
  INTEGER :: i,k,m,n
  CHARACTER(256) :: filename='file/mesp/yyyymmddhh.grd'

  CALL ensmean_grd(member,nij1,v3d,v2d,v3dm,v2dm)

  CALL gather_grd_mpi(0,v3dm,v2dm,v3dg,v2dg)
  IF(myrank == 0) THEN
    WRITE(filename,'(2A,I10.10,a)') trim(base),"/mean/",ymdh,".grd"
    WRITE(6,'(A,I3.3,2A)') 'MYRANK ',myrank,' is writing a file ',TRIM(filename)
    CALL write_grd4(TRIM(filename),v3dg,v2dg)
  END IF

  !==> KLD based on KK
  IF( TRIM(base) /= "anal" )THEN ! KLD occasionaly does not work for LPFs, maybe due to sorting
  CALL enskldv_grd(member,nij1,v3d,v2d,v3dk,v2dk)
  CALL gather_grd_mpi(0,v3dk,v2dk,v3dg,v2dg)
  IF(myrank == 0) THEN
    WRITE(filename,'(2A,I10.10,a)') trim(base),"/kldv/",ymdh,".grd"
    WRITE(6,'(A,I3.3,2A)') 'MYRANK ',myrank,' is writing a file ',TRIM(filename)
    CALL write_grd4(TRIM(filename),v3dg,v2dg)
  END IF
  END IF
  ...
  !==> CRPS Spread (crsp)
  !!!CALL enscrsp_grd(member,nij1,v3d,v2d,v3dk,v2dk)
  !!!CALL gather_grd_mpi(0,v3dk,v2dk,v3dg,v2dg)
  !!!IF(myrank == 0) THEN
  !!!  WRITE(filename,'(2A,I10.10,a)') trim(base),"/crsp/",ymdh,".grd"
  !!!  WRITE(6,'(A,I3.3,2A)') 'MYRANK ',myrank,' is writing a file ',TRIM(filename)
  !!!  CALL write_grd4(TRIM(filename),v3dg,v2dg)
  !!!END IF
...
!$OMP END PARALLEL DO
  ...
  IF(myrank == 0) THEN
    WRITE(filename,'(2A,I10.10,a)') trim(base),"/sprd/",ymdh,".grd"
    WRITE(6,'(A,I3.3,2A)') 'MYRANK ',myrank,' is writing a file ',TRIM(filename)
    CALL write_grd4(TRIM(filename),v3dg,v2dg)
  END IF
