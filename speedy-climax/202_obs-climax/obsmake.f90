...
!$USE OMP_LIB
  ...
  DO
    ...
    IF(ios /= 0) EXIT
    DO n=1,nobkind
      wk(7)=REAL(ityp_ADPUPA_obs) ! treated as adpupa
      ...
      ELSE
        ...
        DO k=1,nlev
          !CLIMAX!wk(4)=REAL(v2d(i,j,iv2d_ps)*sig(k),r_sngl)/100.0 ! hPa
          wk(4)=plv(k)/100.0 ! hPa ; p-level is specified for climax
          nn = nn+1
          SELECT CASE(obelm(n))
          CASE(id_u_obs)
