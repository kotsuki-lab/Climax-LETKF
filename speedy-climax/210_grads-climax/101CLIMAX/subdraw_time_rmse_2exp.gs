...
'rms1=sqrt(aave(pow((T.1-T.2),2),globe))' ; 'spr1=sqrt(aave(pow((    T.4),2),globe))'
'rms2=sqrt(aave(pow((T.1-T.3),2),globe))' ; 'spr2=sqrt(aave(pow((    T.5),2),globe))'
...
ivar=1
while(ivar<=4)  
  if(ivar=1|ivar=2); 'set cstyle 1' ; 'set cthick 10' ; endif
  if(ivar=3|ivar=4); 'set cstyle 2' ; 'set cthick  3' ; endif
  if(ivar=1|ivar=3); 'set ccolor 1' ; endif
  if(ivar=2|ivar=4); 'set ccolor 2' ; endif
...
'./gsfile/cbar_line -x 2.75 -y 2.95 -c 2 -m 2 -l 1 -sz 0.27 -t "EXP2"'
