...
for HSG    in ${HSGTBL} ; do
  ...
  if [ $DASTYPE -ge 2 ]; then
    ...
    if [ "$RSM" = "3" ]; then CURRENT_TEST=${CURRENT_TEST}'_infOT' ; fi
    if [ "$RSM" = "4" ]; then CURRENT_TEST=${CURRENT_TEST}'_infVT' ; fi

    if [ "$NRM" = "0" ]; then CURRENT_TEST=${CURRENT_TEST}'_nrmPF' ; fi
    if [ "$NRM" = "1" ]; then CURRENT_TEST=${CURRENT_TEST}'_nrmGM' ; fi
...
##SUCCEED##if [ -d $OUTPUT ] ; then 
##SUCCEED##  #DEBUG#rm -r $OUTPUT
##SUCCEED##  echo "error :: stop not to overwrite results" $OUTPUT
##SUCCEED##  exit
##SUCCEED##fi

#==> add by kotsuki
INTDIR=$SPEEDY/DATA/init_ensemble
if [ $OBS = "raob" ] ; then INTDIR=$SPEEDY/DATA/raob/init_ensemble_M000040L1100IADP ; fi
if [ $OBS = "reg2" ] ; then INTDIR=$SPEEDY/DATA/reg2/init_ensemble_M000040L0600IADP ; fi

##SUCCEED#####if [ ! -d $OUTPUT ] ; then cp -ar $INTDIR $OUTPUT ; fi
##SUCCEED##mkdir -p $OUTPUT/gues 
##SUCCEED##mkdir -p $OUTPUT/wvec # SK
##SUCCEED##cd $OUTPUT
##SUCCEED##  cp -ar $INTDIR/anal      ./
##SUCCEED##  cp -ar $INTDIR/anal_f    ./
##SUCCEED##  cp -ar $INTDIR/infl_mul  ./
##SUCCEED##  cp -ar $INTDIR/peff_lpf  ./
##SUCCEED##  cp -ar $INTDIR/rsmp_lpf  ./
##SUCCEED##  cp -ar $INTDIR/asis_lpf  ./
##SUCCEED##  cp -ar $INTDIR/log       ./
##SUCCEED##  cp -ar $INTDIR/gues/mean ./gues/
##SUCCEED##  cp -ar $INTDIR/gues/sprd ./gues/
##SUCCEED##  cp -ar $INTDIR/gues/kldv ./gues/
##SUCCEED##cd $OUTPUT/gues
##SUCCEED##  for MEM in $MEM_list ; do
##SUCCEED##    MEM=$( printf %06i $MEM )
##SUCCEED##    cp -ar   $INTDIR/gues/$MEM ./
##SUCCEED##  done
...
done ; done ; done ; done ; done ; done #RTP   #RLX   #RSM   #NRM   #FGT   #STH
